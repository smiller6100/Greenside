...
export function useRound(code: string) {
  ...
  const sendSetRules = useCallback((token: string, rules: { handicapMode?: string; formats?: Record<string, boolean>; games?: Record<string, boolean>; stakes?: Record<string, number>; teams?: string[] }) => {
